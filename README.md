...
- **关于 QQ 版本（两套独立 QQ）**：两个后端需要**互不兼容**的 QQ 版本，因此镜像内装了**两套独立的官方 Linux QQ**：
  - `/opt/QQ` = **3.2.19-39038**，仅供自建签名服务（SignServer）使用 —— 其内存偏移是针对该精确版本逆向得到的，
    SignServer 只 `dlopen` 它的 `wrapper.node`，从不运行其 Electron 应用。
  - `/opt/QQ-napcat` = **3.2.23-44343**，仅供 NapCatQQ 使用 —— NapCat 4.18.4 的 PacketBackend 要求 QQ 构建号
    **≥ 40768**，并会明确拒绝旧版 3.2.19-39038（报错 `PacketBackend 不支持当前QQ版本架构：3.2.19-39038-x64`），
    故为其单独安装受支持的版本。

  **请勿随意升级任一 QQ 版本**：升级签名用的 QQ 会使签名失效（需同步改 `sign.config.toml` 的 `offset`/`version`），
  升级 NapCat 用的 QQ 需与 NapCat 版本所支持的构建号匹配。如需更换，请相应更新 `Dockerfile` 的
  `QQ_DEB_URL`/`QQ_DEB_SHA512`（签名）或 `QQ_NAPCAT_DEB_URL`/`QQ_NAPCAT_DEB_SHA512`（NapCat）。
...
- **关于镜像体积**：因内置**两套 QQ**（签名版 + NapCat 版）+ Xvfb/ffmpeg + GUI 库 + 自建签名服务，镜像体积约 **3GB**
...
- **外部组件版本与容器内更新**：Lagrange.OneBot（linux-x64 self-contained nightly）、VincentZyu233/SignServer、
  NapCatQQ（Shell `v4.18.4`）、官方 Linux QQ（签名用 `3.2.19-39038` + NapCat 用 `3.2.23-44343`）均在实现时通过来源核实并在 `Dockerfile`
