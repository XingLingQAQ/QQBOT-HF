...
- **NapCat WebUI**：内嵌 NapCat 自带的管理面板（扫码登录、网络配置、日志、调试终端等），经后端反向代理在**同一端口**（`/napcat/*`）下访问，HF 单端口环境开箱可用，无需额外开放端口（仅 NapCat 协议）。
...
│   │   └── webui.json    # NapCat WebUI 配置（环回 6099 + 随机 token，首启生成）
│   ├── cache/
...
8. **NapCat WebUI**：在「NapCat WebUI」页内嵌 NapCat 自带管理面板；仅当协议为 NapCatQQ 且 `napcat` 进程运行时可用，否则给出切换/启动指引。
...
### NapCat WebUI（单端口反向代理）

NapCat 自带一个 WebUI（默认监听 `127.0.0.1:6099`），可用于扫码登录、网络配置、查看日志、调试终端等。由于 HF Space 只暴露一个端口，无法直接访问 6099，面板在后端做了一层反向代理：

- 外部统一经 `/napcat/*` 访问（HTTP + WebSocket 全透传），后端剥掉 `/napcat` 前缀后转发到容器内 `127.0.0.1:6099`，WebUI 服务**从不直接对外暴露**。
- 每个请求都要求**面板登录态**（同源 `qqbot_session` Cookie），未登录返回 401。
- NapCat v4.4+ 已弃用 `prefix` 配置、前端资源走根路径（`/webui/`、`/api`），会与后端 `/api` 冲突；镜像构建时已将其改写为 `/napcat/webui/`、`/napcat/api`，因此与面板自身的 `/api` 不冲突。
- WebUI 登录 token 首次启动时由 entrypoint 随机生成并持久化于 `/data/napcat/config/webui.json`；「NapCat WebUI」页内嵌的链接已带 `?token=` 自动填充。
- 仅当协议为 NapCatQQ 且 `napcat` 进程运行时可用；切换到 Lagrange 或进程未启动时，访问 `/napcat/*` 会返回 502（中文提示）。
...
- **关于 QQ 版本（两后端共用一套 QQ）**：镜像内只装**一套官方 Linux QQ `3.2.23-44343`**，由两个后端共用：
  - **NapCatQQ** 运行这套 QQ 的 Electron 应用。NapCat 4.18.4 的 PacketBackend 要求 QQ 构建号 **≥ 40768**，
    并会明确拒绝旧版（报错 `PacketBackend 不支持当前QQ版本架构：3.2.19-39038-x64`），故采用满足要求的 `3.2.23-44343`。
  - **自建签名服务（SignServer）** 只 `dlopen` 同一套 QQ 的 `wrapper.node`（从不运行其 Electron 应用）。
    SignServer 支持通过 `offset`/`version` 配置适配任意 QQ 构建：本项目已定位 `3.2.23-44343` 的签名函数偏移
    （`offset = 0x535B04E`，由与已知 `3.2.19-39038`（`0x5ADE220`）的签名函数特征逐字节比对得到——指令序列完全一致，
    仅栈帧由 `0x188` 增大到 `0x198`），并随附匹配的 appinfo（`3.2.23-44343.json`，由 NapCat 的 appid 表确认
    `SubAppId=537336639`），因此**一套 QQ 即可同时服务签名与 NapCat**，镜像也只需一套 QQ。详见
    `backend/app/templates/sign.config.toml.template`。

  **请勿随意升级 QQ 版本**：升级 QQ 会使签名失效（需同步改 `sign.config.toml` 的 `offset`/`version` 并提供对应
  `{version}.json` appinfo），且新版需与 NapCat 所支持的构建号匹配。如需更换，请相应更新 `Dockerfile` 的
  `QQ_DEB_URL`/`QQ_DEB_SHA512`。
...
- **关于镜像体积**：因内置官方 QQ + Xvfb/ffmpeg + GUI 库 + 自建签名服务，镜像体积约 **2GB**
  （远大于纯 Lagrange 方案），CI 构建与 HF 首次拉取会相应变慢，属正常现象。
...
- **外部组件版本与容器内更新**：Lagrange.OneBot（linux-x64 self-contained nightly）、VincentZyu233/SignServer、
  NapCatQQ（Shell `v4.18.4`）、官方 Linux QQ（`3.2.23-44343`，签名与 NapCat 共用）均在实现时通过来源核实并在 `Dockerfile`
