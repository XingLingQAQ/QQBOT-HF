...
- **进程总览**：按当前协议实时显示 Lagrange + 签名服务 / NapCat、NoneBot、后端进程状态。
...
1. **总览**：顶部账号卡显示登录状态、QQ 号、昵称与头像；**协议选择器**可在 Lagrange.OneBot 与 NapCatQQ 间一键切换（带确认弹窗）；下方按当前协议展示相关进程的实时状态（每 3 秒轮询）。
